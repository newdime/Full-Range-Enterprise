function date(token, returnData) {
  ...
  var startDate = Utilities.formatDate(date, "GMT+11", "d/MM/yy");
  date.setDate(date.getDate()+(week-1));
  var endDate = Utilities.formatDate(date, "GMT+11", "d/MM/yy");
  date.setDate(date.getDate()-(week-1));
  ...
  var i = 1;
  var sheetArr = [];
  sheetArr[0] = Utilities.formatDate(date, "GMT+11", "d/MM/yy");
  while (i < week)
  {
    var sheetNames = date.setDate(date.getDate()+(1));
    sheetArr[i] = Utilities.formatDate(date, "GMT+11", "d/MM/yy");
    i++
  }
  Logger.log(sheetArr);
